...
class Result {
    public:
        ...
        constexpr inline Result(std::uint32_t code): res(code) { }
        constexpr inline Result(std::uint32_t module, std::uint32_t desc): res((module & 0x1ff) | ((desc & 0x1fff) << 9)) { }

        constexpr inline explicit operator std::uint32_t() const { return this->res; }

        constexpr inline std::uint32_t code()   const { return this->res;}
        constexpr inline std::uint32_t module() const { return (this->res & 0x1ff) | 2000; }
        constexpr inline std::uint32_t desc()   const { return (this->res >> 9) & 0x3fff; }
