...
class Logger {
    ...
    public:
        ...
        static inline void enqueue(Level lvl, const char *fmt, Args &&...args) {
            ...
            printf("[%#.3fs] %s %s",
#ifdef CMW_SWITCH // std::chrono broken on switch
                (float)(armGetSystemTick() - start_time) / armGetSystemTickFreq(),
#else
                std::chrono::duration_cast<std::chrono::duration<float, std::ratio<1, 1000>>>(std::chrono::system_clock::now() - start_time).count(),
#endif
                lvl_strings[(int)lvl],
                fmted.c_str()
            );
